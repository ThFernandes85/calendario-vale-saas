...
-- ============================================================
-- SALA DE CONTROLE (Painel de Correias Transportadoras) na própria Mutuca.
-- Ao contrário de client_approval_workflow/materials_contract (sistemáticas
-- alternativas e mutuamente exclusivas), correias_panel é um painel ADICIONAL
-- que só lê os mesmos equipamentos/agendamentos que o site já tem -- por
-- isso liga direto no site 'MUTUCA' já existente (Limpeza Industrial),
-- em vez de duplicar as áreas/ativos num site novo. Assim, qualquer limpeza
-- que o Encarregado lançar no Calendário da Mutuca já aparece ao vivo na
-- Sala de Controle, sem precisar de nenhuma sincronização.
-- ============================================================
update public.sites set correias_panel = true where key = 'MUTUCA';
